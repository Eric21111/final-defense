...
const sendLowStockAlert = async (ownerEmail, lowStockItems) => {
  ...
  try {
    const result = await sendEmail(ownerEmail, subject, null, htmlContent);
    if (result.success) {
      console.log(`[EmailService] Low stock alert sent to ${ownerEmail}`);
      return { success: true };
    }
    return { success: false, error: result.error ? result.error.message : 'Failed to send stock alert email' };
  } catch (error) {
