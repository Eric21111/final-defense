    ...
    <div className="fixed inset-0 flex items-center justify-center z-9999 p-4 backdrop-blur-sm pointer-events-none">
      <div
        ...
        <form onSubmit={handleFormSubmit}>
          {/* Vertical stepper layout for Add mode */}
          {!editingProduct ? (
            <div className="flex" style={{ maxHeight: "calc(95vh - 120px)" }}>
              {/* ── Left: Vertical Stepper Sidebar ── */}
              <div className={`flex-shrink-0 py-8 pl-6 pr-2 flex flex-col items-center ${theme === "dark" ? "border-gray-700" : "border-gray-100"}`} style={{ width: 140 }}>
                {STEPS.map((step, index) => (
                  <div key={step.id} className="flex flex-col items-center">
                    {/* Circle */}
                    <div
                      onClick={() => { if (currentStep > step.id) setCurrentStep(step.id); }}
                      className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-all duration-200 ${currentStep > step.id
                        ? "bg-green-500 border-green-500 text-white cursor-pointer hover:shadow-md"
                        : currentStep === step.id
                          ? "bg-[#AD7F65] border-[#AD7F65] text-white shadow-lg"
                          : theme === "dark"
                    ...
                    {index < STEPS.length - 1 && (
                      <div className={`w-[2px] my-1 rounded transition-all duration-200 ${currentStep > step.id ? "bg-green-500" : theme === "dark" ? "bg-gray-700" : "bg-gray-200"}`} style={{ height: 32 }} />
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="flex-1 flex flex-col min-w-0">
                <div className="flex-1 px-8 pb-4 pt-6 overflow-y-auto">
            ...
            /* Edit mode: original scrollable layout */
            <>
